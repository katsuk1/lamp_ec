...
/**
 * postで送信された値をセッションにセット
 * 
 * @param str $name postで送信された値、セッション変数名
 * @param str $value セッション変数にセットする値
 */
 function set_session_post($name){
   if(isset($_POST[$name])){
     set_session($name, $_POST[$name]);
   }
 }
 
 /**
  * 指定した値がセッションに保存されていればセッションから削除
  * 
  * @param $name str セッション変数名
  */
  function isset_delete_session($name){
 ...
 * 
 * @return bool セッション変数にuser_idがセットされている場合true
 */
function is_logined(){
...
function get_upload_filename($file){
  ...
  $is_valid_image = array(
    'jpg',
    'JPG',
    'png',
    'PNG'
  );
  $ext = $is_valid_image[$mimetype];
...
function is_valid_upload_image($image){
  ...
  if(array_key_exists($mimetype, $is_valid_image) === false){
    set_error('ファイル形式は' . implode('、', $is_valid_image) . 'のみ利用可能です。');
    return false;
  }
  return true;
}
